...
class FluxGradientEntryCalculator(BasisGradientL2ProductEntryCalculator):
    ...
    _fast_element: space.QuadratureFastElement
    ...
    def _build_finite_element(self):
        self._fast_element = space.QuadratureFastElement(
...
class AdvectionFluxGradient:
    ...
    def __init__(self, element_space: space.LagrangeSpace):
...
def build_exact_flux_gradient(
    problem: str, element_space: space.LagrangeSpace
...
def build_flux_gradient(
    problem: str, element_space: space.LagrangeSpace, exact_flux: bool
) -> Callable[[np.ndarray], np.ndarray]:
    if exact_flux:
