...
class TestNormalization(TestCase):
    def test_loading(self):
        ...
        torch.save(normalize.state_dict(), "test/pde_solver/network/test_normalize.pth")

        loaded_normalize = network.Normalization()
        loaded_normalize.load_state_dict(
            torch.load("test/pde_solver/network/test_normalize.pth")
        )
...
class TestNetwork(TestCase):
    ...
    def test_loading(self):
        ...
        torch.save(model.state_dict(), "test/pde_solver/network/test_network.pth")

        loaded_model = network.NeuralNetwork()
        loaded_model.load_state_dict(
            torch.load("test/pde_solver/network/test_network.pth")
        )
