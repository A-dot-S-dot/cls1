...
from problem.scalar.solver.mcl import MCLRightHandSide
from problem.scalar.solver.cg_low import LowOrderCGRightHandSide
...
class TestLinearLowOrderCGRightHandSide(TestCase):
    ...
    artificial_diffusion = scalar.DiscreteUpwind(element_space)
    flux_gradient = scalar.ApproximatedFluxGradient(element_space, lambda u: u)
    flux_approximation = scalar.FluxApproximation(lambda u: u)
...
class TestLinearBurgersLowOrderCGRightHandSide(TestLinearLowOrderCGRightHandSide):
    element_space = LINEAR_LAGRANGE_SPACE
    lumped_mass = scalar.LumpedMassVector(element_space)
    artificial_diffusion = scalar.BurgersArtificialDiffusion(element_space)
    flux_gradient = scalar.ApproximatedFluxGradient(
        element_space, lambda u: 1 / 2 * u**2
    )
    flux_approximation = scalar.FluxApproximation(lambda u: 1 / 2 * u**2)
    low_cg = LowOrderCGRightHandSide(lumped_mass, artificial_diffusion, flux_gradient)
    mcl = MCLRightHandSide(element_space, low_cg, flux_approximation)
    test_dofs = [np.array([1, 2, 3, 4])]
