...
class TestNormalization(TestCase):
    def test_loading(self):
        normalize = network.Normalization(torch.Tensor([0]), torch.Tensor([1]))
        input = torch.Tensor([1])
        expected_output = 1

        torch.save(normalize.state_dict(), "test/base/network/test_normalize.pth")

        loaded_normalize = network.Normalization()
        loaded_normalize.load_state_dict(
            torch.load("test/base/network/test_normalize.pth")
        )
...
class TestCurvature(TestCase):
    def test_scalar_input(self):
        ...
        curvature = network.Curvature(0.25)
...
class TestNetwork(TestCase):
    ...
    def test_loading(self):
        ...
        torch.save(model.state_dict(), "test/base/network/test_network.pth")

        loaded_model = network.NeuralNetwork()
        loaded_model.load_state_dict(torch.load("test/base/network/test_network.pth"))
