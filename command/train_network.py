...
import pandas as pd
import torch
from shallow_water.solver.subgrid_network import NeuralNetwork
...
class CustomSubgridFluxDataset(Dataset):
    data: pd.DataFrame
    mean: Sequence[float]
    std: Sequence[float]
    input_dimension: int

    def __init__(self, subgrid_flux_data_path: str):
        self.data = load_data(subgrid_flux_data_path)
...
class TrainNetwork(Command):
    ...
    def __init__(
        ...
        self._loss_graph_path = loss_graph_path or defaults.LOSS_GRAPH_PATH

    def _build_data_loader(
        self, training_data_path: str, validation_data_path: str, batch_size: int
    ):
        training_data = CustomSubgridFluxDataset(training_data_path)
        validation_data = CustomSubgridFluxDataset(validation_data_path)
    ...
    def _plot_losses(self, training_loss, validation_loss):
        ...
        plt.ylabel("Log(Loss)")
        plt.legend()
        plt.savefig(self._loss_graph_path)
        plt.show()
